...
function buildComponent(options, additionalFiles = {}) {
    return (host, context) => __awaiter(this, void 0, void 0, function* () {
        ...
        const defaultZorroComponentOptions = schematics_1.getDefaultComponentOptions(project);
        let modulePrefix = '';
        ...
        const templateSource = schematics_2.apply(schematics_2.url(schematicFilesUrl), [
            options.skipTests ? schematics_2.filter(path => !path.endsWith('.spec.ts.template')) : schematics_2.noop(),
            options.inlineStyle ? schematics_2.filter(path => !path.endsWith('.__style__.template')) : schematics_2.noop(),
            options.inlineTemplate ? schematics_2.filter(path => !path.endsWith('.html.template')) : schematics_2.noop(),
            // Treat the template options as any, because the type definition for the template options
            // is made unnecessarily explicit. Every type of object can be used in the EJS template.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            schematics_2.applyTemplates(Object.assign({ indentTextContent, resolvedFiles }, baseTemplateContext)),
            // TODO(devversion): figure out why we cannot just remove the first parameter
            // See for example: angular-cli#schematics/angular/component/index.ts#L160
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            schematics_2.move(null, parsedPath.path)
        ]);
        return () => schematics_2.chain([
            schematics_2.branchAndMerge(schematics_2.chain([
